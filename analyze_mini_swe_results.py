...
from tg_code.security import scan_code_for_violations
...
def extract_security_violations_from_patch(patch_text: str) -> Tuple[List[str], bool]:
    ...
    try:
        result = scan_code_for_violations(wrapped, active_rules=active_rules)
        return (result.violations, result.scan_failed)
    except SyntaxError:
        return ([], True)
    except Exception:
        return ([], True)
