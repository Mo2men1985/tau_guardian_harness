...
def test_unresolved_instance_with_sad_is_veto(tmp_path: Path) -> None:
    msa_dir = tmp_path / "msa"
    msa_dir.mkdir()
    ...
    +value = random.random()
    """
    preds = [{"instance_id": "demo__proj-4", "model_patch": patch}]
    (msa_dir / "preds.json").write_text(json.dumps(preds), encoding="utf-8")
    _write_exit_status(msa_dir, ["demo__proj-4"], "Submitted")

    instance_results = tmp_path / "instance_results.jsonl"
    instance_results.write_text(
        json.dumps({"instance_id": "demo__proj-4", "resolved": False, "resolved_status": "UNRESOLVED"})
        + "\n",
        encoding="utf-8",
    )
    ...
    rows = [json.loads(ln) for ln in output_path.read_text(encoding="utf-8").splitlines() if ln.strip()]
    assert rows[0]["eval_status"] == "unresolved"
    assert rows[0]["sad_flag"] is True
    assert rows[0]["final_decision"] == "VETO"
